...
// --- Sampling Time ---
const float Ts = 0.02; // 50 Hz

// --- EMA Filter ---
float pressureEMA = 0;
float alpha = 0.05; // smoothing (0.05–0.2 recommended)
...
float Q = 0.005;// Process noise covariance (smoothness)
float R = 80.0;// Measurement noise covariance (noise level)
...
void setup() {
  ...
  TCCR1B = TCCR1B & 0b11111000 | 0x02; // Pin 10 → ~3.9 kHz
  TCCR2B = TCCR2B & 0b11111000 | 0x02; // Pin 11 → ~3.9 kHz
...
void loop() {
  ...
  if (deltaTime < Ts) return;   // control sampling rate
  deltaTime = Ts;
  ...
  */
  ...
  if(pressureRaw <= 0.0){
   ...
   //---KALMAN FILTER---
    float pressureMeasured = kalmanFilter(pressureRaw);  // ✅ FILTERED VALUE
